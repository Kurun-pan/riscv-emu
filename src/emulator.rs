...
use crate::elf_loader::{ElfLoader, EiClass, ShType, EMachine};
...
impl Emulator {
    ...
    pub fn load_program(&mut self, filename: &Path) {
        ...
        for i in 0..progbits_sec_headers.len() {
            if progbits_sec_headers[i].sh_addr >= DRAM_BASE_ADDRESS && progbits_sec_headers[i].sh_offset > 0 {
				for j in 0..progbits_sec_headers[i].sh_size {
                    let data = loader.read8((progbits_sec_headers[i].sh_offset + j) as usize);
                    match self.cpu.mmu.write8(progbits_sec_headers[i].sh_addr + j as u64, data) {
                        Err(e) => panic!("{:?}", e.factor),
                        _ => {},
                    }
				}
			}
